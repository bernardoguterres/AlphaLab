...
def run_sim(
    ...
    for i in range(len(df)):
        ...
        if ts in signals.index:
            ...
            if sv != 0:
                reason = (
                    signals.loc[ts, "reason"] if "reason" in signals.columns else ""
                )
                pending = (sv, reason)
...
def _compute_benchmark(df, capital):
    ...
    return [
        {"date": idx, "value": round(shares * p + leftover, 2)}
            ...
            if position == 0:
                ...
                if r < p["oversold"] and d > p["adx_threshold"]:
                    ...
                    signals.loc[idx, "confidence"] = min(
                        1.0,
                        (p["oversold"] - r) / p["oversold"]
                        + (d - p["adx_threshold"]) / 30,
                    )
        ...
        for i in range(50, len(data)):
            ...
            if position == 0:
                ...
                if h_prev > 0 and h_curr <= 0:
                    ...
                    signals.loc[idx, "reason"] = (
                        f"MACD Hist bearish cross ({h_curr:.4f})"
                    )
                    position = 0
                    last_signal_bar = i
    ...
    for ticker in TICKERS:
        ...
        try:
            result = fetcher.fetch(
                ticker, start_date="2014-01-01", end_date="2024-12-31", interval="1d"
            )
...
def print_results_table(all_results, title):
    ...
    print(f"{'='*90}")
    print(
        f"{'Strategy':<22} {'Ticker':<6} {'Period':<12} {'Sharpe':>7} "
...
def main():
    ...
    print(
        "Problem: Requires new 20-day high + 50% volume surge + RSI>50 simultaneously"
    )
    ...
    for config_name, params in mb_configs:
        ...
        results_post = test_strategy(
            strat, data_map, "post_covid", *PERIODS["post_covid"]
        )
        ...
        print_results_table(
            results_pre + results_post, f"Momentum Breakout - {config_name}"
        )
        agg = aggregate_stats(results_full)
        print(
            f"  AGGREGATE FULL PERIOD: Sharpe={agg['avg_sharpe']:.2f} | "
    ...
    print(
        "Problem: Requires 2σ deviation AND extreme RSI simultaneously (4 trades/8yr)"
    )
    ...
    for config_name, params in vr_configs:
        ...
        print_results_table(
            results_pre + results_post, f"VWAP Reversion - {config_name}"
        )
        agg = aggregate_stats(results_full)
        print(
            f"  AGGREGATE FULL PERIOD: Sharpe={agg['avg_sharpe']:.2f} | "
    ...
    adx_results_pre = test_strategy(
        adx_rsi_strat, data_map, "pre_covid", *PERIODS["pre_covid"]
    )
    adx_results_post = test_strategy(
        adx_rsi_strat, data_map, "post_covid", *PERIODS["post_covid"]
    )
    ...
    for adx_thresh in [15, 25]:
        for oversold in [30, 38]:
            ...
            print(
                f"  Variation adx_thresh={adx_thresh}, oversold={oversold}: "
    ...
    macd_results_pre = test_strategy(
        macd_strat, data_map, "pre_covid", *PERIODS["pre_covid"]
    )
    macd_results_post = test_strategy(
        macd_strat, data_map, "post_covid", *PERIODS["post_covid"]
    )
    ...
    print(
        f"{'Strategy':<20} {'Metric':<10} {'AAPL':>8} {'MSFT':>8} {'SPY':>8} {'QQQ':>8} {'AVG':>8}"
    )
    print(f"{'-'*74}")
    ...
    for name, strat in summary_configs:
        ...
        for metric, label in [
            ...
            row_parts = [
                (
                    f"{vals.get(t, 0):>8.2f}"
                    if metric not in ("total_trades",)
                    else f"{vals.get(t, 0):>8d}"
                )
                for t in tickers
            ]
        ...
        pre_cagrs = [
            pre_results.get(t, {}).get("cagr_pct", 0)
            for t in tickers
            if t in pre_results
        ]
        post_cagrs = [
            post_results.get(t, {}).get("cagr_pct", 0)
            for t in tickers
            if t in post_results
        ]
        ...
        deploy = (
            "DEPLOY"
            if (
                np.mean([full_results.get(t, {}).get("sharpe", 0) for t in tickers])
                >= 1.0
                and sum(full_results.get(t, {}).get("total_trades", 0) for t in tickers)
                >= 20
                and variance <= 100
            )
            else "REVIEW"
        ...
        print(
            f"  {name:<18} {'Pre CAGR':<10} {avg_pre:>8.1f}%   Post CAGR {avg_post:.1f}%   "
